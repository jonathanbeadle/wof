...
class BingoGame {
    ...
    saveResult(number, ballInfo) {
        ...
        try {
            localStorage.setItem('jugBingoRecentDraws', JSON.stringify(this.recentDraws));
    ...
    loadRecentDraws() {
        try {
            const saved = localStorage.getItem('jugBingoRecentDraws');
    ...
    setupModeIntegration() {
        const MODE_STORAGE_KEY = 'jugWheelMode'; // namespaced so this venue never shares state with the main wheel
